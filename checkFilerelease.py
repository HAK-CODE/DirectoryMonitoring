        ...
        if os.path.getsize(filepath) != 0:
            ...
            if os.path.isfile(filepath):
                ...
                if 'SENSOR' in newname:
                    ...
                    subprocess.Popen(
                        ['python3', 'sensorcsvProcessing.py', paths_list[3] + '/' + ntpath.basename(newname)])
                elif 'LOG' in newname:
                    ...
                    subprocess.Popen(
                        ['python3', 'metercsvProcessing.py', paths_list[4] + '/' + ntpath.basename(newname)])
                    ...
                    subprocess.Popen(
                        ['python3', 'invertercsvProcessing.py', paths_list[1] + '/' + ntpath.basename(newname)])
                elif 'Z_UMTS_log' in newname:
                    copy(newname, paths_list[5])
                    subprocess.Popen(
                        ['python3', 'zumtscsvProcessing.py', paths_list[5] + '/' + ntpath.basename(newname)])
                os.remove(newname)
                break
            else:
                time.sleep(1)
